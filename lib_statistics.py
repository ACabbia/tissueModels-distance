...
def split_classes(df):
    ...
    for c in df.columns:
        if c.split('_')[2] == 'train':
            df1[c] = df[c]
        elif c.split('_')[2] == 'untr':
            df2[c] = df[c]
    return df1 , df2
...
def model_stats_plot(rxn,met,gen):
    ...
    R_df = R_df.melt(var_name = "Status", value_name = 'Number of reactions')
    sns.violinplot(x="Status", y="Number of reactions", data=R_df).set_title('Reactions content')
    show()

    M_df["Untrained"] = M_untrained.sum().values
    M_df["Trained"] = M_trained.sum().values
    M_df = M_df.melt(var_name = "Status", value_name = 'Number of metabolites')
    sns.violinplot(x="Status", y="Number of metabolites", data=M_df).set_title('Metabolites content')
    show()

    G_df["Untrained"] = G_untrained.sum().values
    G_df["Trained"] = G_trained.sum().values
    G_df = G_df.melt(var_name = "Status", value_name = 'Number of genes')
    sns.violinplot(x="Status", y="Number of genes", data=G_df).set_title('Genes content')
    show()

    return table_df


path_lib = '/home/acabbia/Documents/Muscle_Model/models/muscle_old_sedentary_traineded/'
