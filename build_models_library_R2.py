...
library_folder =  path + '/muscle_old_sedentary_trained/'
...
requirements_int = ['ATPS4m','ENO','PDHm','PYK','G3PD1','G6PDH2r','AKGDm','CYOOm3','r0913','GLCt2_2']
...
for gsm in serie.gsms:
    ...
    if 'Old' in name and ('T1' in name or 'T3' in name):
        ...
        confidence = Builder.rxn_confidence(ref_model, eset, translator,'hgnc_id', expr_cutoff )
        
        # Ensure requred reactions are included
        for r_id in requirements:
            confidence[r_id] = 3
        
        newmodel = Builder.build_model(ref_model,confidence)
        ...
        cobra.io.write_sbml_model(newmodel,filename= library_folder+'SKM_old'+label[0].split('_')[0]+label[1]+'.xml')
        print('===============================')
